...
function Layout({ children }) {
  ...
  React.useEffect(() => {
    const fetchAlerts = () =>
        apiFetch(`/api/alerts?ts=${Date.now()}`)
            .then((list) => setAlerts(list || []))
            .catch(() => setAlerts([]));
    fetchAlerts();
    const pollTimer = setInterval(fetchAlerts, 2000);
    ...
    const onAlertMessage = (event) => {
      const payload = JSON.parse(event.data);
      setAlerts((prev) => [payload, ...prev].slice(0, 50));
    };
    source.addEventListener("alert", onAlertMessage);
    source.onmessage = onAlertMessage;
    source.onerror = () => {};
    return () => {
      window.removeEventListener("alert:new", onLocalAlert);
      source.removeEventListener("alert", onAlertMessage);
      source.close();
